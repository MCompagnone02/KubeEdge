...
The "edge" refers to the boundary between the physical world and the digital infrastructure — it includes devices such as:

- Industrial sensors and actuators on factory floors
- Cameras and vision systems for real-time analysis
- IoT gateways in smart buildings or cities
- Vehicles and drones with onboard processing
- Retail point-of-sale systems
- Medical devices in hospitals or in the field
...
Many edge environments cannot guarantee continuous network access:

- A ship at sea may have satellite connectivity for only a few hours per day
- A wind turbine in a remote area may lose connectivity during storms
- A mobile vehicle moves in and out of network coverage

A cloud-dependent system would simply stop working in these scenarios. Edge systems must be designed to operate autonomously when disconnected and resync when connectivity is restored.
