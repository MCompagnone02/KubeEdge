...
**What happened**: K3s started a single-node Kubernetes cluster using SQLite as the data store. The kubeconfig was copied to `$HOME/.kube/config` so `kubectl` can find it without specifying `--kubeconfig` on every command.
...
**Common issue**: copy-pasting YAML from the terminal may introduce tabs instead of spaces. To avoid this, transfer the `deployment-nginx.yaml` file from your KubeEdge folder using `multipass transfer`.
